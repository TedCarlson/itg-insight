...
export async function POST(req: NextRequest) {
  ...
  if (!report) return json(404, { ok: false, error: "Commercial Battery report not found." });
  if (report.category_key !== "commercial_battery_billing") {
    return json(400, { ok: false, error: "Report is not Commercial Battery Billing." });
  }
  if (report.status !== "approved") {
    return json(400, { ok: false, error: "Only approved Commercial Battery records can be prepared for billing." });
  }
  ...
  if (!canPrepareBilling(accessPass)) {
    return json(403, { ok: false, error: "Not allowed to prepare Commercial Battery billing packet." });
  }
