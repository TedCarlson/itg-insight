...
const EVIDENCE = [
  { key: "workorder_screenshot", label: "Work Order Screenshot" },
  { key: "battery_serial_number", label: "Battery Serial Number" },
] as const;
...
async function buildPdf(args: {
  ...
  drawText(page, "Commercial Battery Billing Packet", margin, y, 18, bold);
  y -= 20;
  drawText(page, "Single Job Billing Packet", margin, y, 10, font, rgb(0.35, 0.35, 0.4));
  drawText(page, "Approved Commercial Battery", 330, y, 10, bold);
  ...
  for (const row of args.rows) {
    ...
    const metaY = top - 42;
    drawText(page, `Type: Commercial Battery • ${(row.job_type ?? "—").toUpperCase()}`, margin + 12, metaY, 8, font);
    ...
    drawText(page, `Notes / Billing Code: ${row.comment?.trim() || "—"}`, margin + 12, metaY - 12, 8, font);

    const gridTopY = metaY - 30;
...
export async function GET(req: NextRequest) {
  ...
  const { data: rowData, error: rowError } = await supabase
    ...
    .select("report_id,job_number,job_type,status,submitted_at,approved_at,subject_full_name,subject_tech_id,gps_lat,gps_lng,gps_accuracy_m,location_captured_at,comment,pc_org_id,category_key")
  ...
  if (!rowData) {
    return NextResponse.json({ ok: false, error: "Commercial Battery report not found." }, { status: 404 });
  }

  if (rowData.category_key !== "commercial_battery_billing") {
    return NextResponse.json({ ok: false, error: "Report is not Commercial Battery Billing." }, { status: 400 });
  }

  if (rowData.status !== "approved") {
    return NextResponse.json({ ok: false, error: "Only approved Commercial Battery records can generate billing packets." }, { status: 400 });
  }
  ...
  const filename = `CommercialBattery_${jobNumber}.pdf`;
