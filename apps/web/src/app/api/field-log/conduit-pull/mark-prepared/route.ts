...
export async function POST(req: NextRequest) {
  ...
  if (!report) return json(404, { ok: false, error: "Conduit Pull report not found." });
  if (report.category_key !== "conduit_pull_install") {
    return json(400, { ok: false, error: "Report is not a Conduit Pull." });
  }
  if (report.status !== "approved") {
    return json(400, { ok: false, error: "Only approved Conduit Pulls can be prepared for billing." });
  }
  ...
  if (!canPrepareBilling(accessPass)) {
    return json(403, { ok: false, error: "Not allowed to prepare Conduit Pull billing packet." });
  }
