...
const EVIDENCE = [
  ...
  { key: "conduit_line_entry_photo", label: "Conduit Line Entry Photo" },
  { key: "conduit_line_exit_photo", label: "Conduit Line Exit Photo" },
] as const;
...
async function buildPdf(args: {
  ...
  drawText(page, "Conduit Pull Billing Packet", margin, y, 18, bold);
  y -= 20;
  drawText(page, "Conduit Pull Install Packet", margin, y, 10, font, rgb(0.35, 0.35, 0.4));
  drawText(page, "Approved Conduit Pull", 330, y, 10, bold);
  ...
  for (const row of args.rows) {
    ...
    const metaY = top - 42;
    drawText(page, `Type: Conduit Pull Install • ${(row.job_type ?? "—").toUpperCase()}`, margin + 12, metaY, 8, font);
...
export async function GET(req: NextRequest) {
  ...
  if (!rowData) {
    return NextResponse.json({ ok: false, error: "Conduit Pull report not found." }, { status: 404 });
  }

  if (rowData.category_key !== "conduit_pull_install") {
    return NextResponse.json({ ok: false, error: "Report is not a Conduit Pull." }, { status: 400 });
  }

  if (rowData.status !== "approved") {
    return NextResponse.json({ ok: false, error: "Only approved Conduit Pulls can generate billing packets." }, { status: 400 });
  }
  ...
  const filename = `ConduitPull_${jobNumber}.pdf`;
