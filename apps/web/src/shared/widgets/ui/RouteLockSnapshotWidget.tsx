...
function MetricTile(props: { item: HomeMetricDatum }) {
  return (
    <div className={`rounded-xl border px-3 py-2 ${toneClass(props.item.tone)}`}>
      ...
      <div className="mt-1 text-xl font-semibold">{props.item.value}</div>
      {props.item.note ? (
        <div className="mt-1 text-xs opacity-75">{props.item.note}</div>
      ) : null}
    </div>
  );
}

function AlertRow(props: { item: HomeActivityItem }) {
  return (
    <div className={`rounded-xl border px-3 py-2 ${toneClass(props.item.tone)}`}>
      <div className="text-sm font-medium">{props.item.label}</div>
      {props.item.detail ? (
        <div className="mt-1 text-xs opacity-75">{props.item.detail}</div>
      ) : null}
    </div>
  );
}
    ...
    <div className="flex h-full flex-col">
      ...
      {!isSmall && props.payload.links?.length ? (
        <div className="mt-auto flex flex-wrap gap-2 pt-4">
          {props.payload.links.slice(0, 1).map((link) => (
            <a
              key={link.href}
              href={link.href}
