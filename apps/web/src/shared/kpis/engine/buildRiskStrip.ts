...
function hasMetricValue(row: WorkforceRow) {
  return row.metrics.some((metric) => metric.value != null);
}

function hasPositiveJobs(row: WorkforceRow) {
  return (row.work_mix?.total ?? 0) > 0;
}

function hasVisibleData(row: WorkforceRow) {
  return hasPositiveJobs(row) || hasMetricValue(row);
}

export function buildRiskStrip(args: {
  rosterRows: WorkforceRow[];
  kpis: KpiCfg[];
}): RiskStripItem[] {
