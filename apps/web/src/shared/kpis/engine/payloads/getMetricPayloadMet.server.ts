// path: apps/web/src/shared/kpis/engine/payloads/getMetricPayloadMet.server.ts
...
export async function getMetricPayloadMet(
  ...
  const rows = (rawRows ?? [])
    ...
    .filter(
      (row: any) => String(row.metric_key ?? "").trim() === "met_rate"
    )
    .map((row: any) => ({
      ...
      met_count: toNum(row.numerator),
      total_appts: toNum(row.denominator),
      met_rate: toNum(row.metric_value),
  ...
  return {
    ...
    summary: {
      met_rate: latest.met_rate,
      met_count: latest.met_count,
      total_appts: latest.total_appts,
    },
    trend: rows,
  };
}