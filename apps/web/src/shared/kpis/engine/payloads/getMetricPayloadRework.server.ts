// path: apps/web/src/shared/kpis/engine/payloads/getMetricPayloadRework.server.ts
...
export async function getMetricPayloadRework(
  ...
  const rows = (rawRows ?? [])
    ...
    .filter(
      (row: any) => String(row.metric_key ?? "").trim() === "rework_rate"
    )
    .map((row: any) => ({
      ...
      rework_count: toNum(row.numerator),
      total_appts: toNum(row.denominator),
      rework_rate: toNum(row.metric_value),
  ...
  return {
    ...
    summary: {
      rework_rate: latest.rework_rate,
