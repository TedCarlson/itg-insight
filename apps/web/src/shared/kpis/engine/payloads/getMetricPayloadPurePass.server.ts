// path: apps/web/src/shared/kpis/engine/payloads/getMetricPayloadPurePass.server.ts
...
export async function getMetricPayloadPurePass(
  ...
  const rows = (rawRows ?? [])
    ...
    .filter(
      (row: any) =>
        String(row.metric_key ?? "").trim() === "pht_pure_pass_rate"
    )
    .map((row: any) => ({
      ...
      pht_jobs: toNum(row.denominator),
      pure_pass: toNum(row.numerator),
      pure_pass_rate: toNum(row.metric_value),
  ...
  return {
    ...
    summary: {
      pure_pass_rate: latest.pure_pass_rate,
      pht_jobs: latest.pht_jobs,
      pure_pass: latest.pure_pass,
    },
    trend: rows,
  };
}