// path: apps/web/src/shared/kpis/engine/payloads/getMetricPayloadSoi.server.ts
...
export async function getMetricPayloadSoi(
  ...
  const rows = (rawRows ?? [])
    ...
    .filter((row: any) => String(row.metric_key ?? "").trim() === "soi_rate")
    .map((row: any) => ({
      ...
      soi_count: toNum(row.numerator),
      installs: toNum(row.denominator),
      soi_rate: toNum(row.metric_value),
  ...
  return {
    ...
    summary: {
      soi_rate: latest.soi_rate,
      soi_count: latest.soi_count,
