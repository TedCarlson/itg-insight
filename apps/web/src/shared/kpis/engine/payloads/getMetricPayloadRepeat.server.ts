// path: apps/web/src/shared/kpis/engine/payloads/getMetricPayloadRepeat.server.ts
...
export async function getMetricPayloadRepeat(
  ...
  const rows = (rawRows ?? [])
    ...
    .filter(
      (row: any) => String(row.metric_key ?? "").trim() === "repeat_rate"
    )
    .map((row: any) => ({
      ...
      repeat_count: toNum(row.numerator),
      tc_count: toNum(row.denominator),
      repeat_rate: toNum(row.metric_value),
  ...
  return {
    ...
    summary: {
      repeat_rate: latest.repeat_rate,
      repeat_count: latest.repeat_count,
      tc_count: latest.tc_count,
    },
    trend: rows,
  };
}