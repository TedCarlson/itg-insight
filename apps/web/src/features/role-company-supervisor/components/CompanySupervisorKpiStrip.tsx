...
export type CompanySupervisorKpiItem = {
  kpi_key: string;
  label: string;
...
type Props = {
  items: CompanySupervisorKpiItem[];
};
...
function comparisonPillClass(
  state: CompanySupervisorKpiItem["comparison_state"]
) {
  if (state === "better") {
...
function comparisonLine(item: CompanySupervisorKpiItem) {
  return `${item.comparison_scope_code} ${item.comparison_value_display}`;
}

function PrimaryKpiCard({ item }: { item: CompanySupervisorKpiItem }) {
  return (
...
function SecondaryKpiCard({ item }: { item: CompanySupervisorKpiItem }) {
  return (
...
export default function CompanySupervisorKpiStrip({ items }: Props) {
      ...
      <div className="mb-2 flex items-center justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-[10px] text-muted-foreground">
            Supervisor scope compared against total region fact set.
          </div>
        </div>
