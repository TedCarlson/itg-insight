...
// apps/web/src/features/field-log/components/FieldLogTechReviewActionsCard.tsx

"use client";

type FieldLogTechReviewActionsCardProps = {
  busy: boolean;
  canApprove: boolean;
  xmAllowed: boolean;
  xmDeclared: boolean;
...
export function FieldLogTechReviewActionsCard(
  props: FieldLogTechReviewActionsCardProps,
) {
  const {
    busy,
    canApprove,
  ...
  return (
    <section className="rounded-2xl border bg-card p-5">
      <div className="text-base font-semibold">Tech Submission Review</div>

      <div className="mt-3 rounded-xl border bg-muted/30 p-3 text-sm">
        <div className="font-medium">Review Check</div>
          ...
          <div className="mt-2 space-y-1 text-muted-foreground">
            ...
            {photoRequirementMet ? (
              <div className="font-medium text-green-600">
                Photo requirement met. Ready to close.
              </div>
            ) : (
              <div className="font-medium text-amber-600">
                {photoGap} more photo{photoGap === 1 ? "" : "s"} required before closing.
              </div>
            )}
          </div>
        )}
        ...
        <div className="mt-3 space-y-3">
          ...
          {xmLinkValid ? (
            <div className="text-sm font-medium text-green-600">
              XM link validated. Ready to close.
            </div>
          ) : xmLink.trim().length > 0 ? (
            <div className="text-sm font-medium text-blue-600">
              XM link entered. Closing will validate and append it.
            </div>
          ) : (
            <div className="text-sm font-medium text-red-600">
              XM link required before closing.
            </div>
          )}
        </div>
      ) : null}
      ...
      <div className="mt-3 space-y-2">
        ...
        <textarea
          ...
          placeholder="Add a short note for closure or follow-up…"
          rows={4}
          className="w-full rounded-xl border px-3 py-3"
        />
        <div className="text-xs text-muted-foreground">
          Recommended for all tech review actions. Required for follow-up requests.
        </div>
      </div>
